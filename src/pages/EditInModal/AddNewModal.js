/**
 * NOTE: 1、火车或飞机 - 只有9
 * NOTE: 2、无 - 航空/铁路（不可填，税率根据实际填写其他内容判定）
 * NOTE: 3、无 - 公路、水路 - 9
 * NOTE: 4、无 - 出租车 - 0369
 * NOTE: 5、无 - 住宿 - 036
 * NOTE: 6、轮船 - 公路、水路 - 3
...
class AddNewModal extends React.Component {
    ...
    taxCodeClicked = (e) => {
        ...
        console.log('taxCodeCliked e.key:', e.key)
        ...
        if (record.CabinType === '0') {
            ...
            this.setState({
                ...
                this.setState({
                    ...
                    _FORM.setFieldsValue({
                        ExpenseHotelTaxCode: record['ExpenseHotelTaxCode']
                    })
                })
            })
        }
            ...
            if (parseFloat(record['ExpenseBoat']) !== 0) {
                ...
                this.setState({
                    ...
                    if (!this.compareArray(this.state.taxCodes, newTaxCodes)) {
                        ...
                        // 还要重新设定记录的taxcode是新数组中的第一条，否则会报错！！！
                        // FIXME: 编辑一条数据点击确定以后再次点击编辑，数据会因为此条语句被修改
                        // record.ExpenseHotelTaxCode = newTaxCodes[0].key;
                        record.ExpenseHotelTaxCode = !record.ExpenseHotelTaxCode ? newTaxCodes[0].key : record.ExpenseHotelTaxCode;
                        this.setState({
                            record,
                            taxCodes: newTaxCodes
                        }, () => {
